...
def print_info(gen_auroc, gen_auprc, dis_auroc, dis_auprc, epoch=-1, dis_loss=0, gen_loss=0):
    print("Epoch:", '%04d' % (epoch + 1),
          ...
          "dis_auroc=", "{:.5f}".format(dis_auroc),
          "dis_auprc=", "{:.5f}".format(dis_auprc))


def deal_filename(dataset, fold):
    # 数据部分
    config.train_filename = "../../data/datasets/{0}/{1}fold/{0}_train.txt".format(dataset, fold)
    config.test_filename = "../../data/datasets/{0}/{1}fold/{0}_test.txt".format(dataset, fold)
    config.test_neg_filename = "../../data/datasets/{0}/{1}fold/{0}_test_neg.txt".format(dataset, fold)
    config.pretrain_emb_filename_d = "../../data/datasets/{0}/{1}fold/{0}_pre_train.emb".format(dataset, fold)
    config.pretrain_emb_filename_g = "../../data/datasets/{0}/{1}fold/{0}_pre_train.emb".format(dataset, fold)
    config.initial_features = "../../data/datasets/{0}/feature/{0}_feature.pkl".format(dataset)
...
def read_features(dataset):
    pickle_file = open("../../data/datasets/{0}/feature/{0}_feature.pkl".format(dataset), "rb")
